...
class MainVendas(Ui_ct_MainVendas, Ui_ct_FormVenda, DataAtual):
    ...
    def FormVendas(self):
        ...
        # Receber
        self.bt_GerarParcela.clicked.connect(self.gerarParcela)
    ...
    def validaCad(self):
        ...
        elif self.tb_Parcelas.rowCount() < 1:
            self.cb_ParcelamentoVenda.setFocus()
    ...
    def gerarParcela(self):
        ...
        for i in range(numParcela):
            ...
            # self.conteudoTabela(self.tb_Parcelas, i, 0, str(i + 5))
            # self.conteudoTabelaLeft(self.tb_Parcelas, i, 1, QtCore.QDate.toString(
    ...
    def Receber(self, id):
        ...
        if self.tb_Parcelas.cellWidget(id, 3).text():
            ...
            # if float(self.tb_Parcelas.cellWidget(id, 3).text().replace(",", ".")) < float(self.tb_Parcelas.item(id, 2).text().replace(",", ".")):
            #     INSERI.status = 2
            # else:
            #     INSERI.status = 1
