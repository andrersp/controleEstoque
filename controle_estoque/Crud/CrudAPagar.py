...
class CrudAPagar(object):
    ...
    def selectAPagarId(self):
        ...
        try:
            c.execute(
                """ SELECT contasAPagar.*, status_pagamento.status_pagamento,
                ...
                contasAPagar.formapagamento = formaPagamento.id 
                 WHERE idCompra = '{}' """.format(self.idCompra))
            row = c.fetchall()
            ...
            for lista in row:
                ...
                self.idCompra.append(lista[1])
                self.idFornecedor.append(lista[2])
                ...
                self.dataPagamento.append(lista[9])
